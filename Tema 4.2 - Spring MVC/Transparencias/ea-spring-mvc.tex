...
\subsection{Elementos JPA}

\begin{frame}[c]
    \frametitle{Anotaciones Rest}
    \begin{tabular}{lp{0.70\linewidth}}
        \ann{@PersistenceUnit}    & Datos de acceso a un almacén persistente. \\
        \ann{@PersistenceContext} & \emph{Identity Map} asociado a una \emph{Persistence Unit}. \\
        \ann{EntityManager}       &  Unidad de Trabajo (asociado a un @PersistenceContext) \\
    \end{tabular}
\end{frame}

\begin{frame}[c]
    \frametitle{Elementos JPA}
    \begin{center}
        \includegraphics[width=\linewidth,keepaspectratio=true]{images/jpa/jpaLayers.eps}
    \end{center}
\end{frame}
