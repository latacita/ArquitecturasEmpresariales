...
\begin{frame}[c]
    ...
    \begin{block}{Problema}
        \begin{enumerate}
            ¿Cómo mapeo \emph{value objects} de manera eficiente?
        \end{enumerate}
    \end{block}
    \uncover<3->{
        \begin{block}{Solución}
            Si una clase $C$ tiene una asociación de multiplicidad máximana $1$ con un \emph{value object } $V$, puedo simplemente añadir los campos de $V$ a la clase $C$ y luego mapear $C$ como una clase simple.
        \end{block}
    }
\end{frame}
...
\subsection{Inheritance Mapping}
